...
public class AdminJobEmplymentStatus {
	...
	@Test(priority=1)
		public void addEmpStatus() {
			...
			driver.findElement(By.linkText("Employment Status")).click();//click on Employment status link
			
			driver.findElement(By.xpath("//input[@id='btnAdd']")).click();//Click on add button
		
			driver.findElement(By.xpath("//input[@id='empStatus_name']")).sendKeys("PartTimeTest");//Enter Employment Status
		...
		@Test(priority=2)
	    public void updateEmpStatus() throws InterruptedException{
		   ...
		   for(int i=2;i<elementSize;i++){  //Using for loop
	    	...
	    	if(listOfEmpStatus.contains("PartTimeTest")){	
			...
			driver.findElement(By.xpath("//input[@id='empStatus_name']")).sendKeys("TestUpdate");//Enter Name
